...
componentDidMount=()=>{
  ...
  findTranslation = () => {
    this.state.questions.map((item) => {
      this.fetchApitranslate(item.question, this.state.primaryLanguage, false);
    });
  };

  fetchApitranslate = async(question,language, speech) => {
    const url =
  ...
  speak= async(language,text)=>{
    var result = await this.fetchApitranslate(text, language, true);
    if(result){
    const start =()=>{
